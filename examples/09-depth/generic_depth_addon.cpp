...
static void draw_settings_overlay(effect_runtime *runtime)
{
	...
	const std::vector<std::string> draw_stats_heuristic_items = {
		_("Default"),
		_("Higher vertices"),
		_("Higher draw calls")
	};
	std::vector<const char *> draw_stats_heuristic_items_c;
	for(auto &str : draw_stats_heuristic_items)
		draw_stats_heuristic_items_c.push_back(str.c_str());
	if (ImGui::Combo(_("Draw stats heuristic"), reinterpret_cast<int *>(&s_draw_stats_heuristic), &draw_stats_heuristic_items_c[0], static_cast<int>(std::size(draw_stats_heuristic_items_c))))
	{
		reshade::set_config_value(nullptr, "DEPTH", "DrawStatsHeuristic", static_cast<unsigned int>(s_draw_stats_heuristic));
		force_reset = true;
	}

	const std::vector<std::string> aspect_ratio_heuristic_items = {
		_("None"),
		_("Similar aspect ratio"),
		_("Multiples of resolution (for DLSS or resolution scaling)"),
		_("Match resolution exactly"),
		_("Match custom width and height exactly")
	};
	std::vector<const char *> aspect_ratio_heuristic_items_c;
	for (auto &str : aspect_ratio_heuristic_items)
		aspect_ratio_heuristic_items_c.push_back(str.c_str());
	if (ImGui::Combo(_("Aspect ratio heuristic"), reinterpret_cast<int *>(&s_aspect_ratio_heuristic), &aspect_ratio_heuristic_items_c[0], static_cast<int>(std::size(aspect_ratio_heuristic_items_c))))
	{
	...
	if (s_aspect_ratio_heuristic == aspect_ratio_heuristic::match_custom_resolution_exactly)
	{
		if (ImGui::InputInt2(_("Filter by width and height"), reinterpret_cast<int *>(s_custom_resolution_filtering)))
		{
	...
	if (ImGui::Combo(_("Filter by depth buffer format"), reinterpret_cast<int *>(&s_format_filtering), depth_format_items, static_cast<int>(std::size(depth_format_items))))
	{
