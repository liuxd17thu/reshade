...
namespace ReShade.Setup
{
	...
	{
		public static string CN2Version = @"CN2-v0.73";
		...
		void InstallStep_InstallReShadeModule()
		{
			...
			if (currentInfo.targetApi == Api.Vulkan || currentInfo.targetOpenXR)
			{
				...
				foreach (string layerModuleName in new[] { "ReShade32", "ReShade64" })
				{
					...
					try
					{
						var module = zip.GetEntry(Path.GetFileName(layerModulePath)) ?? throw new FileFormatException("安装包缺少ReShade DLL主程序。");
					...
					if (currentInfo.targetApi == Api.Vulkan)
					{
						...
						try
						{
							var manifest = zip.GetEntry(Path.GetFileName(layerManifestPath)) ?? throw new FileFormatException("安装包缺少Vulkan层清单文件。");
					...
					if (currentInfo.targetOpenXR)
					{
						...
						try
						{
							var manifest = zip.GetEntry(Path.GetFileName(layerManifestPathXR)) ?? throw new FileFormatException("安装包缺少OpenXR层清单文件。");
			...
			else
			{
				...
				try
				{
					var module = zip.GetEntry(currentInfo.is64Bit ? "ReShade64.dll" : "ReShade32.dll") ?? throw new FileFormatException("安装包缺少ReShade DLL主程序。");
