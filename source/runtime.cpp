	...
	for (effect &effect : _effects)
	{
		...
		for (uniform &variable : effect.uniforms)
		{
			...
			case reshadefx::type::t_int:
				...
				if (std::string binding { variable.annotation_as_string("ui_bind") }; !binding.empty()) {
					effect.definition_bindings[binding] = std::to_string(values.as_int[0]);
				}
				break;
			...
			case reshadefx::type::t_uint:
				...
				if (std::string binding { variable.annotation_as_string("ui_bind") }; !binding.empty()) {
					effect.definition_bindings[binding] = std::to_string(values.as_uint[0]);
				}
				break;
			case reshadefx::type::t_float:
				...
				if (std::string binding { variable.annotation_as_string("ui_bind") }; !binding.empty()) {
					effect.definition_bindings[binding] = std::to_string(values.as_float[0]);
				}
				break;
			}
		}
	...
	for (size_t i = 0, array_length = (variable.type.is_array() ? variable.type.array_length : 1); i < array_length; ++i)
	{
		...
		switch (variable.type.base)
		{
			case reshadefx::type::t_int:
			{
				set_uniform_value(variable, value.as_int, variable.type.components(), i);
				if(i == 0)
					ui_bind = std::to_string(value.as_int[0]);
				break;
			}
			case reshadefx::type::t_bool:
			case reshadefx::type::t_uint:
			{
				set_uniform_value(variable, value.as_uint, variable.type.components(), i);
				if (i == 0)
					ui_bind = std::to_string(value.as_uint[0]);
				break;
			}
			case reshadefx::type::t_float:
			{
				set_uniform_value(variable, value.as_float, variable.type.components(), i);
				if (i == 0)
					ui_bind = std::to_string(value.as_float[0]);
				break;
			}
		}
	}
